...
export class AxisHandler {
  private leftPadding = 40;
  private rightPadding = 40;
  ...
  public draw(startX: number, endX: number, minY: number, maxY: number, tickCount = 10): void {
    ...
    this.drawAxes(coordinateSystem);
    this.drawTickMarks(coordinateSystem, tickCount);
    this.drawAxisLabels(coordinateSystem);
   ...
   */
  private drawAxes(coordinateSystem: CoordinateSystem): void {
    ...
    // horizontal axis
    this.context.moveTo(this.leftPadding, axisY);
    this.context.lineTo(this.canvas.width - this.rightPadding, axisY);
    // vertical axis
    this.context.moveTo(axisX, this.topPadding);
    this.context.lineTo(axisX, this.canvas.height - this.bottomPadding);
    this.context.stroke();
  }
  ...
  private drawTickMarks(coordinateSystem: CoordinateSystem, tickCount: number): void {
    ...
    for (const xValue of xTicks) {
      ...
      if (maxX > 10000 || maxX < 0.1) {
        ...
        this.context.fillText(this.formatTick(xValue), xTransformed - 10, axisY + 30);
      }
    }
    ...
    for (const yValue of yTicks) {
      ...
      this.context.beginPath();
      this.context.moveTo(axisX - 6, yTransformed);
      this.context.lineTo(axisX, yTransformed);
      this.context.stroke();
      if (yValue === 0) {
        continue; // Skip labeling the origin
      }
      this.context.fillText(this.formatTick(yValue), axisX - 5, yTransformed + 15);
    }
   ...
   */
  private drawAxisLabels(coordinateSystem: CoordinateSystem): void {
    this.context.fillStyle = 'black';
